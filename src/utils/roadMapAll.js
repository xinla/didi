...
export function initRoadMap (_history, id, boo, boo1, yuce_result) {
  ...
  if (history != null || history != undefined) {
    ...
    drawImage(a_contexts[id], jsonarray, table_size, icon_size, roadmap(history, 32), b_contexts[id], c_contexts[id], d_contexts[id], id, yuce_result);
  }
}

//画大圆圈
function drawImage ( a_context, jsonarray, table_size, icon_size, roadmaps, b_context, c_context, d_context, id, yuce_result) {
  var row_array = []; //列的个数
  var tab_size_small;
  ...
  img.onload = function () {
    ...
    if (!b_context) { return };
    drawSmallRoadMap(b_context, tab_size_small, icon_size, roadmap(hollowArray, 11), 0);
    drawSmallRoadMap(c_context, tab_size_small, icon_size, roadmap(solid_array, 11), 1);
    drawSmallRoadMap(d_context, tab_size_small, icon_size, roadmap(slash_array, 11), 2);
  }
