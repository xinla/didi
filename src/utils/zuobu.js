...
Gamezuo.prototype = {
  ...
  events: function () {
    ...
    var container = this.element
    //高亮
    $(container).off('mousemove').on('mousemove', function (evt) {
      _this.movequ = istupian(evt.offsetX, evt.offsetY)
    });
    $(container).off('mouseout').on('mouseout', function (evt) {
    ...
    for (var key in group_list) {
      ...
      if (!_ismianshui && key == 'bet_super') {
        ...
        if (group && group.childCount() > 0) {
          var num = parseInt(group.childOfName('one').style.moneytxt)
          this.zr.remove(group)
  ...
  xiazhu: function (curr_qu, _num, flag, curr_index, curr_addnum, notDan, isshow) {
    ...
    if (_group.childCount() > 0) {
      console.log(parseInt(_group.childOfName('one').style.moneytxt), addnum)
      num = _num ? _num : parseInt(_group.childOfName('one').style.moneytxt) + addnum;
      _group.removeAll()
  ...
  creategroup: function (group, index, qu, num, flag) {
    ...
    if (qu !== 'bet_long' && qu !== 'bet_hu' && qu !== 'bet_xian' && qu !== 'bet_zhuang') {
      if (_group.childOfName('one')) {
        num = parseInt(_group.childOfName('one').style.moneytxt) + num;
        _group.childOfName('one').attr({ style: { text: num, moneytxt: num } })
      ...
      $.each(num_list, function (index, val) {
        ...
        _group.add(_this.listmoney(x, s_y -= 8, val));
      })
  ...
  showResult: function (name) {
    ...
    var _this = this;
    this[name] && this[name].animate('style', false)
      .when(1000, { opacity: 0 })
      .when(2000, { opacity: .5 })
      .when(3000, { opacity: 0 })
      .when(4000, { opacity: .5 })
