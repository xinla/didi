...
export function drawCanvas (a_canvas1, a_canvas, e_canvas1, e_canvas, id) {
  ...
  var cell_width = e_canvas.width / grid_cols;
...
export function initRoadMap (_history, yuce_result) {
  ...
  var table_size = a_canvas_h / 12;//表格大小
  var history = _history.slice(0, _history.length);

  paintingTypeface(_history, e_context1);
  ...
  if (history != null || history != undefined) {
    ...
    drawImage(context1, jsonarray, table_size, icon_size, roadmap(history, 40), yuce_result);
  }
}
...
function drawImage (a_context, jsonarray, table_size, icon_size, roadmaps, yuce_result) {
  ...
  img.onload = function () {
    ...
    if (yuce_result) {
      yuce_result[gameId] = yuCe(z_yuce, x_yuce)
    ...
    if (!a_context) { return };
    drawSmallRoadMap(a_context, tab_size_small, icon_size, roadmap(hollowArray, 80), 0);
    drawSmallRoadMap(a_context, tab_size_small, icon_size, roadmap(solid_array, 40), 1);
    drawSmallRoadMap(a_context, tab_size_small, icon_size, roadmap(slash_array, 40), 2);
    return yuce_result
  }
...
function drawSmallRoadMap (context, table_size, icon_size, roadmap, type) {
  var drawing_size = table_size * 0.99; //画图大小
  var img = new Image();
  img.src = src;
  var c = 0;
  var r = 0;
  ...
  for (c in roadmap) {
    for (r in roadmap[c]) {
      switch (type) {
        ...
        case 2: {
          context.clearRect(c * drawing_size + drawing_size * 40, r * drawing_size + drawing_size * 18, drawing_size, drawing_size);
          break;
        };
        default: {
          break;
      ...
      if (roadmap[c][r] != "0000") {
        ...
        switch (type) {
          ...
          case 2: {
            ...
            obj = json[LZtype].coor;
            context.drawImage(img, obj.x, obj.y, icon_size, icon_size, c * drawing_size + drawing_size * 40, r * drawing_size + drawing_size * 18, drawing_size, drawing_size);
            break;
          };
          default: {
            break;
...
function paintingTypeface (history, context) {
  var icon_size = 84; //图标大小
  var table_size = e_canvas_h / 6; //表格大小
